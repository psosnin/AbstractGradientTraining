...
N_SEEDS = 2  # number of seeds to test
LOSS_FNS = [
...
class TestBounds(unittest.TestCase):
    ...
    def initialise_attributes(self, shape, epsilon, batchsize, loss, seed):
        ...
        if loss == "binary_cross_entropy":
            ...
            self.loss = F.binary_cross_entropy_with_logits(
                self.logit.flatten(start_dim=1).double(), self.labels[:, None].double(), reduction='none'
            )
            self.grad_bound = loss_gradient_bounds.bound_bce_derivative
        elif loss == "hinge":
            # compute hinge loss
            hinge_labels = 2 * self.labels - 1
            self.loss = F.hinge_embedding_loss(self.logit.flatten(start_dim=1), hinge_labels[:, None], reduction='none')
            self.grad_bound = loss_gradient_bounds.bound_hinge_derivative
        else:
    ...
    def test_backward_passes(self, shape, epsilon, batchsize, loss):
        ...
        if shape[-1] == 1 and loss not in ["binary_cross_entropy", "hinge"]:
            return
        if shape[-1] > 1 and loss in ["binary_cross_entropy", "hinge"]:
            return
        for seed in range(N_SEEDS):
            self.initialise_attributes(shape, epsilon, batchsize, loss, seed)
