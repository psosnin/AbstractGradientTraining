...
class AGTConfig:
    ...
    optimizer: str = pydantic.Field("sgd", in_=OPTIMIZERS.keys())
    optimizer_kwargs: Dict = pydantic.Field(default_factory=dict)
    loss: str = pydantic.Field(..., in_=LOSS_BOUNDS.keys())
    device: str = "cpu"
    log_level: str = pydantic.Field("INFO", in_=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"])
    # bounding parameters
    forward_bound: str = pydantic.Field("interval", in_=FORWARD_BOUNDS.keys())
    backward_bound: str = pydantic.Field("interval", in_=BACKWARD_BOUNDS.keys())
    ...
    def __post_init__(self):
        ...
        if self.fragsize <= k:
            raise ValueError("fragsize must be greater than k but got fragsize={} and k={}".format(self.fragsize, k))
