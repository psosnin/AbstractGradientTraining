...
def unlearning_certified_training(
    ...
    for n, (batch, labels) in enumerate(training_iterator, 1):
        ...
        grads_n = [torch.zeros_like(p) for p in bounded_model.param_n]  # nominal gradient
        ...
        for batch_frag, label_frag in zip(batch_fragments, label_fragments):
            ...
            for i in range(len(grads_n)):
                ...
                top_k_l = torch.topk(frag_grads_l[i], min(size, config.k_unlearn), largest=True, dim=0)[0]
                ...
                # the remaining k gradients are stored until all the frags have been processed
                bottom_k_u = torch.topk(frag_grads_u[i], min(size, config.k_unlearn), largest=False, dim=0)[0]
        ...
        for i in range(len(grads_n)):
            ...
            assert grads_l_top_ks_i.size(0) >= config.k_unlearn, "Not enough samples left after processing fragments."
            top_k_l = torch.topk(grads_l_top_ks_i, config.k_unlearn, largest=True, dim=0)[0]
            ...
            bottom_k_u = torch.topk(grads_u_bottom_ks_i, config.k_unlearn, largest=False, dim=0)[0]
            grads_u[i] += grads_u_bottom_ks_i.sum(dim=0) - bottom_k_u.sum(dim=0)
            del grads_u_bottom_ks_i

        # normalise each by the batchsize
        grads_l = [g / (batch.size(0) - config.k_unlearn) for g in grads_l]
        grads_n = [g / batch.size(0) for g in grads_n]
        grads_u = [g / (batch.size(0) - config.k_unlearn) for g in grads_u]
