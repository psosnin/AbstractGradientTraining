...
if __name__ == "__main__":
    ...
    for j in range(len(custom_grads)):
        print(
            f"Layer {j}: matching gradients = {torch.allclose(custom_grads[j], autograds_intermediate[j], 1e-6, 1e-6)} ",
            f"{(custom_grads[j] - autograds_intermediate[j]).abs().max()}",
        )
    ...
    for j in range(len(custom_grads)):
        print(
            f"Parameter {j}: matching grads = {torch.allclose(custom_grads[j], autograds_parameters[j], 1e-6, 1e-6)}",
            f"{(autograds_parameters[j] - custom_grads[j]).abs().max()}",
        )  # type: ignore
